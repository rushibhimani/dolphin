 ...
 * Compact table-style rows grouped by date.
 */
...
async function renderUpcoming(){
  ...
  try{
    ...
    Object.keys(byDate).sort().forEach(d => {
      ...
      const rows = dops.map(op => {
        ...
        return `<tr style="border-bottom:1px solid var(--border)">
          <td style="padding:7px 12px;white-space:nowrap">
            ...
            ${op.priority?'<span style="color:var(--red);font-size:11px;margin-left:4px">🚨</span>':''}
          </td>
          <td style="padding:7px 8px;font-weight:600;font-size:13px">${op.op_name}</td>
          <td style="padding:7px 8px;font-size:12px;color:var(--muted)">${op.wc_name}</td>
          <td style="padding:7px 8px;font-size:12px;color:var(--muted)">${op.worker_name||'—'}</td>
          <td style="padding:7px 8px;font-size:11px;color:var(--muted)">${op.customer}</td>
      ...
      html += `<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden;margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:var(--surface);border-bottom:1px solid var(--border)">
          <span style="font-weight:700;font-size:14px">${dayLabel}</span>
          <span style="font-size:12px;color:var(--muted);font-family:var(--mono)">${dops.length} ops</span>
        </div>
        <div style="overflow-x:auto">
          <table style="width:100%;border-collapse:collapse;min-width:480px">
            <thead><tr style="background:var(--surface)">
              ...
              <th style="padding:5px 8px;font-size:10px;color:var(--muted);text-align:left;font-weight:600">Machine</th>
...
// ── (expandedJobId and jobNextOps declared in today.js) ──
