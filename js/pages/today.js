 ...
 * Compact single-line rows, actual vs estimated time for in-progress ops.
 */
...
async function renderToday(){
  ...
  try{
    ...
    function renderOpRow(op){
      ...
      // Actual elapsed time for in-progress ops
      let actualHtml = '';
      if(isNow && op.actual_start){
        const elapsedMs  = Date.now() - new Date(op.actual_start).getTime() - (5.5*3600000); // IST offset
    ...
    let html = `<div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
      <div style="background:var(--accent-soft);border:1px solid var(--accent);border-radius:8px;padding:6px 14px;font-size:13px;display:flex;align-items:center;gap:6px"><b style="font-size:18px;color:var(--accent)">${active.length}</b> In Progress</div>
      <div style="background:var(--amber-soft,rgba(245,158,11,.08));border:1px solid var(--amber);border-radius:8px;padding:6px 14px;font-size:13px;display:flex;align-items:center;gap:6px"><b style="font-size:18px;color:var(--amber)">${paused.length}</b> Paused</div>
      <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:6px 14px;font-size:13px;display:flex;align-items:center;gap:6px"><b style="font-size:18px">${sched.length}</b> Scheduled</div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(480px,1fr));gap:14px">`;
